#+TITLE: aetheria project progress
...
** TODO security
*** DONE figure out gpg
keygen, ecc, subkeys, gpg-agent, ssh-agent, retroactively signing this
repository
*** TODO sops-guix
https://github.com/fishinthecalculator/sops-guix
ideally, the only stateful secret should be my gpg key
*** TODO fix ssh pinentry in emacs
magit/eshell create new tty per each operation, and the pinentry tty isn't
updated in the gpg-agent. have no idea how to go about fixing this, other
than dropping tty-based pinentry entirely? but pinentry-tty is /very/
accessible.
+ ~gpg-connect-agent UPDATESTARTUPTTY /bye~ expects ~$GPG_TTY~ to be set.
+ attempting to set it inside ~~/.ssh/config~ is pointless, as ~exec~ in the
  config isn't evaluated with a tty.
+ currently im setting it in ~\~/.bashrc~, which only works for pty
  allocations which hand over to bash. eshell/magit don't.
+ maybe attempt to wrap ~ssh~?
...
mechanism.
+ [X] ~%aetheria-desktop-home-packages~ from ~(aetheria home base)~
...
** DONE make ~(aetheria users ...)~ procedures that take in hostname
